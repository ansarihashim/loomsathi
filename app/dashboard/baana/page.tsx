...
interface Baana {
  _id: string
  baana_arrival_date: string
  baana_arrived: number
  week_start: string
  week_end: string
  createdAt: string
  updatedAt: string
...
const BaanaDashboard = () => {
  const router = useRouter()
  const [isLoading, setIsLoading] = useState(true)
  const [baanas, setBaanas] = useState<Baana[]>([])
  const [filteredBaanas, setFilteredBaanas] = useState<Baana[]>([])
  const [searchTerm, setSearchTerm] = useState('')
  const [isModalOpen, setIsModalOpen] = useState(false)
  const [isDeleteModalOpen, setIsDeleteModalOpen] = useState(false)
  const [selectedBaana, setSelectedBaana] = useState<Baana | null>(null)
  const [formData, setFormData] = useState<BaanaFormData>({
    baana_arrival_date: '',
    baana_arrived: 0,
    week_start: '',
    week_end: ''
  ...
  const handleCreate = () => {
    ...
    setRawWeekStart('');
    setRawWeekEnd('');
    setSelectedBaana(null)
    setError('')
    setIsModalOpen(true)
  }

  const handleEdit = (baana: Baana) => {
    setSelectedBaana(baana)
    // Convert ISO dates to DD/MM/YYYY for the raw input
    const arrivalDate = baana.baana_arrival_date.split('T')[0]
    const weekStart = baana.week_start.split('T')[0]
    const weekEnd = baana.week_end.split('T')[0]
    setRawArrivalDate(formatDate(arrivalDate))
    setRawWeekStart(formatDate(weekStart))
    setRawWeekEnd(formatDate(weekEnd))
    setFormData({
      baana_arrival_date: arrivalDate,
      baana_arrived: baana.baana_arrived,
  ...
  const handleDelete = (baana: Baana) => {
    setSelectedBaana(baana)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorText = await response.text()
        console.error('Error response:', errorText)
        let errorMessage = 'Failed to save baana'
        try {
      ...
      console.error('Error saving baana:', error)
      setError(error.message || 'Failed to save baana')
    } finally {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error('Failed to delete baana')
      }
      ...
      console.error('Error deleting baana:', error)
      setError('Failed to delete baana')
    } finally {
      stopLoading()
    }
  }
    ...
    return (
      <AuthGuard>
        <div className="min-h-screen bg-gray-50 flex items-center justify-center">
          <LoadingSpinner message="Loading baana dashboard..." size="lg" />
        </div>
      </AuthGuard>
    )
  }
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-4">
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <h1 className="text-xl lg:text-2xl font-bold text-gray-900">Baana Management</h1>
                </div>
              </div>
              ...
              >
                ...
                <span className="hidden sm:inline">Add Baana</span>
              </button>
            </div>
          </div>
        </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {filteredBaanas.length > 0 ? (
                    filteredBaanas.map((baana) => (
                      <motion.tr
                        key={baana._id}
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                      ...
                      >
                        <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-900">
                          {formatDate(baana.baana_arrival_date)}
                        </td>
                        <td className="px-4 py-4 whitespace-nowrap text-sm font-medium text-purple-600">
                          {baana.baana_arrived}
                        </td>
                        <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-500">
                          {formatDate(baana.week_start)} - {formatDate(baana.week_end)}
                        </td>
                        <td className="px-4 py-4 whitespace-nowrap text-sm font-medium">
                          <div className="flex items-center space-x-2">
                            <button
                              onClick={() => handleEdit(baana)}
                              className="p-1 text-blue-600 hover:text-blue-800 transition-colors"
                            >
                              <Edit className="w-4 h-4" />
                            </button>
                            <button
                              onClick={() => handleDelete(baana)}
                    ...
                    <tr>
                      <td colSpan={4} className="px-4 py-8 text-center text-gray-500">
                        {searchTerm ? 'No baana found matching your search.' : 'No baana records found.'}
                      </td>
                    </tr>
                  )}
                </tbody>
                ...
                <div className="px-6 py-4 border-b border-gray-200">
                  <div className="flex items-center justify-between">
                    <h2 className="text-lg font-semibold text-gray-900">
                      {selectedBaana ? 'Edit Baana' : 'Add New Baana'}
                    </h2>
                    <button
                      onClick={() => setIsModalOpen(false)}
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        required
                        min="1"
                        value={formData.baana_arrived}
                        onChange={(e) => setFormData(prev => ({ ...prev, baana_arrived: parseInt(e.target.value) }))}
                        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent"
                      />
                    </div>
                    ...
                    >
                      {isSubmitting ? (
                        ...
                        <>
                          ...
                          <span>{selectedBaana ? 'Update' : 'Create'}</span>
                        </>
                      )}
                    </button>
                  </div>
              ...
              >
                ...
                <div className="px-6 py-4" style={{ maxHeight: 'calc(85vh - 140px)', overflowY: 'auto', paddingRight: '8px' }}>
                  <p className="text-gray-600 mb-4">
                    Are you sure you want to delete this baana record? This action cannot be undone.
                  </p>
                  
                  {selectedBaana && (
                    <div className="bg-gray-50 p-3 rounded-lg mb-4">
                      <p className="text-sm text-gray-700">
                        <span className="font-medium">Quantity:</span> {selectedBaana.baana_arrived}
                      </p>
                      <p className="text-sm text-gray-700">
                        <span className="font-medium">Arrival Date:</span> {formatDate(selectedBaana?.baana_arrival_date)}
                      </p>
                    </div>
                  )}
                </div>
